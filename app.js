...
function pushSharedState(message) {
  if (!collaboration.sharedMap || collaboration.isApplyingRemote) {
    return;
  }

  collaboration.isPushingLocal = true;
  const snapshot = buildSnapshot();
  collaboration.doc.transact(() => {
    collaboration.sharedMap.set("snapshot", JSON.stringify(snapshot));
    collaboration.sharedMap.set("updatedAt", Date.now());
  });
  collaboration.isPushingLocal = false;

  persistLocalSnapshot(snapshot, message || `Live layout synced at ${formatTimestamp()}.`);
}

function pullSharedState(message) {
  if (!collaboration.sharedMap) {
    return false;
  }

  const raw = collaboration.sharedMap.get("snapshot");
  if (!raw) {
    return false;
  }
...
function persistState(message) {
  if (collaboration.enabled && collaboration.sharedMap) {
...
function destroyCollaboration() {
  ...
  collaboration.provider = null;
  collaboration.doc = null;
  collaboration.sharedMap = null;
  collaboration.connected = false;
  collaboration.enabled = false;
  collaboration.loading = null;
}

async function loadCollaborationModules() {
  if (!collaboration.loading) {
    collaboration.loading = Promise.all([
      import("https://esm.sh/yjs@13.6.24"),
      import("https://esm.sh/y-webrtc@10.3.0"),
    ]).then(([yjsModule, webrtcModule]) => ({
      Y: yjsModule,
...
async function ensureCollaboration(roomId) {
  ...
  setSessionStatus(`Connecting to live room ${roomId}...`);
  renderHud();

  try {
    const { Y, WebrtcProvider } = await loadCollaborationModules();
    const doc = new Y.Doc();
    const provider = new WebrtcProvider(`booth-designer-${roomId}`, doc);
    const sharedMap = doc.getMap("layout");

    collaboration.doc = doc;
    collaboration.provider = provider;
    collaboration.sharedMap = sharedMap;
    collaboration.enabled = true;

    sharedMap.observeDeep(() => {
      if (!collaboration.isPushingLocal) {
        pullSharedState();
      }
    });

    provider.on("status", (event) => {
      collaboration.connected = event.status === "connected";
      setSessionStatus(
        collaboration.connected
          ? `Live room ${roomId} is connected. Everyone on this URL edits the same layout.`
          : `Live room ${roomId} is reconnecting...`
      );
    });

    if (!pullSharedState("Live layout loaded from the shared room.")) {
      pushSharedState("Live room created and ready to share.");
    }

    setSessionStatus(`Live room ${roomId} is ready. Anyone on this URL can edit.`);
    return true;
  } catch (error) {
    console.error(error);
    setSessionStatus("Live collaboration could not start. Host this app online so the browser can load collaboration modules.");
    collaboration.enabled = false;
    return false;
  }
}
...
if (initialRoomId) {
  ...
  setSessionStatus("This layout is local until you create a live room link.");
} else {
